...
export async function up(knex: Knex): Promise<void> {
  ...
  const cw721Contracts = await CW721Contract.query(knex).withGraphFetched(
    'smart_contract'
  );
  if (cw721Contracts.length > 0) {
    ...
    await CW721Contract.query(knex)
      .insert(
  ...
  const cw20Contracts = await Cw20Contract.query(knex).withGraphFetched(
    'smart_contract'
  );
  if (cw20Contracts.length > 0) {
    cw20Contracts.forEach((cw20Contract) => {
      cw20Contract.smart_contract_address = cw20Contract.smart_contract.address;
    });
    await Cw20Contract.query(knex)
      .insert(
