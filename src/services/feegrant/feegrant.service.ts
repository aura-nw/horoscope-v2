...
export default class HandleFeegrantHistoryService extends BullableService {
  ...
  async jobHandler(): Promise<void> {
    ...
    await knex.transaction(async (trx) => {
      ...
      const newHistories = feegrantEvents
        .filter((event) => event.action !== FEEGRANT_ACTION.CREATE)
        .map((feegrantEvent) =>
      ...
      await Promise.all([
        Feegrant.query()
          .insertGraph(newFeegrants)
          // .onConflict(['init_tx_id', 'granter', 'grantee'])
          // .merge()
          .transacting(trx),
        FeegrantHistory.query()
          .insert(newHistories)
          // .onConflict(['tx_id', 'granter', 'grantee'])
          // .merge()
          .transacting(trx),
